...
for i in range(num_datasets):
    ...
    corn_hec = np.random.normal(100, 10, total_rows)
    soybeans_hec = np.random.normal(80, 25, total_rows)
    corn_pix = np.random.normal(200, 30, total_rows)
    soybeans_pix = np.random.normal(150, 25, total_rows)
    vardir = np.random.normal(1, 0.2, total_rows)

    mean_corn_pix_per_seg = np.random.normal(200, 10, num_counties)
    mean_soybeans_pix_per_seg = np.random.normal(150, 25, num_counties)
    ...
    df.to_csv(f"bangkitan/pseudo/pseudo_{i+1}.csv", index=False)