...
from controller.Eksploration.EksplorationController import LinePlotController
...
class LinePlotDialog(QDialog):
    ...
    def generate_r_script(self): 
        ...
        if method == "Single Lineplot":
            for var in selected_var_vertical:
                # Generate script untuk setiap variabel vertikal
                r_script += f"""
    # Line plot for {selected_var_horizontal} vs. {var}
        lineplot_{var} <- ggplot(data, aes(x = {selected_var_horizontal}, y = {var})) +
        geom_line(color = sample(colors(), 1)) +  
        ggtitle("Line Plot: {selected_var_horizontal} vs. {var}") +
        xlab("{selected_var_horizontal}") +
        ylab("{var}") +
        theme_minimal()
        """

        # Multiple Line Plot: Gabungkan kolom-kolom vertikal ke dalam satu variabel y
            ...
            r_script += f"""
    # Multiple line plot for {selected_var_horizontal} vs. multiple y variables
    ...
    # Buat line plot
    lineplot_multiple <- ggplot(data_long, aes(x = {selected_var_horizontal}, y = value, color = variable)) +
        geom_line() +
        ggtitle("Multiple Line Plot: {selected_var_horizontal} vs. {', '.join(selected_var_vertical)}") +
        xlab("{selected_var_horizontal}") +
        ylab("Value") +
        theme_minimal()
        """
